...
def recommend_train(day_start_val):
    ...
    col = "index_group_no"
    feat_names = df_art[col].unique().tolist()

    cust2age = {x: y for x, y in zip(df_cust.index, df_cust["age_id_1"].values)}
    # cust2us = {}
    # top_art_age_us_dict = {}
    top_art_age_dict = {}
    for i in tqdm(range(len(day_start))):

        df_trains_date = df_trans[df_trans.t_dat <= day_end[i]].copy()
        feat = (
            df_trains_date.groupby(["customer_id", col])["article_id"]
            .count()
            .unstack()
            .reset_index()
            .fillna(0.0)
        )
        feat[f"{col}_cnt"] = feat[feat_names].sum(axis=1)
        feat[f"{col}_argmax"] = feat[feat_names].idxmax(axis=1)
        feat.loc[feat[f"{col}_cnt"] < 3, f"{col}_argmax"] = 0

        df_cust_tmp = df_cust[["age_id_1"]].merge(
            feat[["customer_id", f"{col}_argmax"]], on="customer_id", how="left"
        )
        df_cust_tmp["index_group_no_argmax"] = (
            df_cust_tmp["index_group_no_argmax"].fillna(0.0).astype(int)
        )
        us_df = (
            df_cust_tmp.groupby(["age_id_1", "index_group_no_argmax"])["customer_id"]
            .count()
            .reset_index()
            .reset_index()
            .rename(columns={"index": "us_1"})
            .drop(columns=["customer_id"])
        )
        df_cust_tmp = df_cust_tmp.merge(us_df, on=["age_id_1", "index_group_no_argmax"])

        # cust2us[i] = {x: y for x, y in df_cust_tmp[["customer_id", "us_1"]].values}

        q_date = f"((t_dat >= '{day_start[i] - datetime.timedelta(days=7*slide_week)}') and (t_dat <= '{day_end[i]}'))"
        df_trans_candi_term = df_trans.query(q_date)

        # top_art_age_us_dict[i] = {}
        # for us in df_cust_tmp.us_1.unique():
        #     df_trans_candi_term_us = df_trans_candi_term[
        #         df_trans_candi_term.customer_id.isin(
        #             df_cust_tmp[df_cust_tmp.us_1 == us]["customer_id"].values
        #         )
        #     ]
        #     top_art_all = (
        #         df_trans_candi_term_us.groupby("article_id")["t_dat"]
        #         .count()
        #         .sort_values(ascending=False)
        #         .index[:N]
        #         .tolist()
        #     )
        #     top_art_age_us_dict[i][us] = top_art_all

        top_art_age_dict[i] = {}
        for age_id_1 in df_cust_tmp.age_id_1.unique():
            df_trans_candi_term_us = df_trans_candi_term[
                df_trans_candi_term.customer_id.isin(
                    df_cust_tmp[df_cust_tmp.age_id_1 == age_id_1]["customer_id"].values
                )
            ]
            top_art_all = (
                df_trans_candi_term_us.groupby("article_id")["t_dat"]
            ...
            top_art_age_dict[i][age_id_1] = top_art_all

    # scaler = MinMaxScaler([1, 100])
    # random_sample_weights = np.array([[i] for i in range(N, 0, -1)])
    # random_sample_weights = scaler.fit_transform(random_sample_weights).T[0]
    ...
    for i in range(len(day_start)):
        ...
        top_art_all = []
        for x in top_art_age_dict[i].values():
            top_art_all.extend(x)
        top_art_all = set(list(top_art_all))

        list_df_buy.append(
            df_trans[df_trans.article_id.isin(top_art_all)]
    ...
    for iter_train in tqdm(range(n_iter)):
        ...
        for i in range(len(day_start)):
            rows = []
            for cid in list_list_cust[i]:
                for aid in random.sample(top_art_age_dict[i][cust2age[cid]], nobuy):
                    rows.append([cid, aid])
            df = pd.DataFrame(rows, columns=["customer_id", "article_id"])
            list_df_nobuy.append(df)
...
def recommend_pred(series_cust, day_start_val, eval_mid=True, sub_i=-1):
    ...
    list_sl = list(range(0, infN, N_div))
    if list_sl[-1] != infN:
        list_sl.append(infN)
