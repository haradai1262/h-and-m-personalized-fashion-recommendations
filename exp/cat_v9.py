...
def feat_store(df_trans, l_cust, ds, de, dsr, der, dsh, dsh_short, dsh_mid, deh):
    ...
    # Item
    art_buy_hist = df_trans_hist.groupby(["article_id"])["t_dat"].agg(
        art_buy_hist="count"
    )
    art_buy_hist["art_buy_hist_rank"] = art_buy_hist["art_buy_hist"].rank(
        method="dense", ascending=False
    )
    feat["art_buy_hist"] = art_buy_hist
    ...
    art_buy_recent = df_trans_recent.groupby(["article_id"])["t_dat"].agg(
        art_buy_recent="count"
    )
    art_buy_recent["art_buy_recent_rank"] = art_buy_recent["art_buy_recent"].rank(
        method="dense", ascending=False
    )
    feat["art_buy_recent"] = art_buy_recent
    art_buy_yesterday = df_trans_yesterday.groupby(["article_id"])["t_dat"].agg(
        art_buy_yesterday="count"
    )
    art_buy_yesterday["art_buy_yesterday_rank"] = art_buy_yesterday[
        "art_buy_yesterday"
    ].rank(method="dense", ascending=False)
    feat["art_buy_yesterday"] = art_buy_yesterday
    ...
    code_buy_recent = df_trans_recent.groupby(["product_code"])["t_dat"].agg(
        code_buy_recent="count"
    )
    code_buy_recent["code_buy_recent_rank"] = code_buy_recent["code_buy_recent"].rank(
        method="dense", ascending=False
    )
    feat["code_buy_recent"] = code_buy_recent
    code_buy_yesterday = df_trans_yesterday.groupby(["product_code"])["t_dat"].agg(
        code_buy_yesterday="count"
    )
    code_buy_yesterday["code_buy_yesterday_rank"] = code_buy_yesterday[
        "code_buy_yesterday"
    ].rank(method="dense", ascending=False)
    feat["code_buy_yesterday"] = code_buy_yesterday
