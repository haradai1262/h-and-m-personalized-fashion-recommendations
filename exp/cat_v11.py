...
def cnt_rank_stats(
    ...
    cnt_df = (
        df_trans.groupby([agg_key])["t_dat"]
        .count()
        .reset_index()
        .rename(columns={"t_dat": col_name})
    )
    cnt_df[f"{col_name}_rank"] = cnt_df[col_name].rank(method="dense", ascending=False)
    if len(attrs) > 0:
        cnt_df = fast_left_join(cnt_df, df_art[attrs], on=agg_key)
        for attr in attrs:
            cnt_df[f"{attr}_{col_name}_rank"] = cnt_df.groupby(attr)[col_name].rank(
                method="dense", ascending=False
            )
        cnt_df = cnt_df.drop(columns=attrs)

    cnt_df.index = cnt_df[agg_key]
    cnt_df.index.name = agg_key
    cnt_df = cnt_df.drop(columns=[agg_key])
    return cnt_df


def trans_attr_feat(name, col, df_art, df_trans, sm=3):

    col_unique = df_art[col].unique()

    feat = (
        df_trans.groupby(["customer_id", col])["article_id"]
        .count()
        .unstack()
        .reset_index()
        .fillna(0.0)
    ...
    for c in col_unique:
        if c not in feat.columns:
            feat[c] = 0.0
    feat[f"{name}_{col}_cnt"] = feat[col_unique].sum(axis=1)
    feat[f"{name}_{col}_argmax"] = feat[col_unique].idxmax(axis=1)

    for c in col_unique:
        feat[f"{name}_{col}_{c}_norm_sm"] = (feat[c] + sm) / (
            feat[f"{name}_{col}_cnt"] + sm * len(col_unique)
        )

    feat_cols = [f"{name}_{col}_argmax"] + [
        f"{name}_{col}_{c}_norm_sm" for c in col_unique
    ]
    feat.index = feat["customer_id"]
    feat = feat[feat_cols]
...
def feat_store(
    df_trans, df_art, l_cust, ds, de, dsr, der, dsh, dsh_short, dsh_mid, deh
):
    feat = {}
    ...
    df_trans_hist_short_ch1 = df_trans_hist_short[
        df_trans_hist_short.sales_channel_id == 1
    ]
    df_trans_yesterday_ch1 = df_trans_yesterday[
        df_trans_yesterday.sales_channel_id == 1
    ]
    df_trans_recent_ch1 = df_trans_recent[df_trans_recent.sales_channel_id == 1]
    df_trans_hist_ch2 = df_trans_hist[df_trans_hist.sales_channel_id == 2]
    df_trans_hist_short_ch2 = df_trans_hist_short[
        df_trans_hist_short.sales_channel_id == 2
    ]

    # >>> All Trans

    # Item
    feat["art_buy_hist"] = cnt_rank_stats(
        "art_buy_hist",
        "article_id",
        df_trans_hist,
        df_art,
        attrs=["perceived_colour_value_id", "index_group_no", "section_no"],
    )
    feat["art_buy_hist_short"] = cnt_rank_stats(
        "art_buy_hist_short",
        "article_id",
        df_trans_hist_short,
        df_art,
    ...
    feat["art_buy_hist_mid"] = cnt_rank_stats(
        "art_buy_hist_mid",
        "article_id",
        df_trans_hist_mid,
        df_art,
    ...
    feat["art_buy_recent"] = cnt_rank_stats(
        "art_buy_recent",
        "article_id",
        df_trans_recent,
        df_art,
    ...
    feat["art_buy_yesterday"] = cnt_rank_stats(
        "art_buy_yesterday",
        "article_id",
        df_trans_yesterday,
        df_art,
    ...
    # Code
    feat["code_buy_hist"] = cnt_rank_stats(
        "code_buy_hist",
        "product_code",
        df_trans_hist,
        df_art,
    ...
    feat["code_buy_recent"] = cnt_rank_stats(
        "code_buy_recent",
        "product_code",
        df_trans_recent,
        df_art,
    ...
    feat["code_buy_yesterday"] = cnt_rank_stats(
        "code_buy_yesterday",
        "product_code",
        df_trans_yesterday,
        df_art,
    ...
    feat["art_buy_hist_short_ch1"] = df_trans_hist_short_ch1.groupby(["article_id"])[
        "t_dat"
    ].agg(art_buy_hist_short_ch1="count")
    feat["art_buy_hist_short_ch2"] = df_trans_hist_short_ch2.groupby(["article_id"])[
        "t_dat"
    ].agg(art_buy_hist_short_ch2="count")
    ...
    age_id_n_buy_hist = df_trans_hist.groupby(["age_id", "article_id"])["t_dat"].agg(
        age_id_n_buy_hist="count"
    )
    age_id_n_buy_hist["age_id_n_buy_hist_rank"] = age_id_n_buy_hist[
        "age_id_n_buy_hist"
    ].rank(method="dense", ascending=False)
    feat["age_id_n_buy_hist"] = age_id_n_buy_hist

    age_id_n_buy_recent = df_trans_recent.groupby(["age_id", "article_id"])[
        "t_dat"
    ].agg(age_id_n_buy_recent="count")
    age_id_n_buy_recent["age_id_n_buy_recent_rank"] = age_id_n_buy_recent[
        "age_id_n_buy_recent"
    ].rank(method="dense", ascending=False)
    feat["age_id_n_buy_recent"] = age_id_n_buy_recent
    ...
    df_trans_hist_ch1 = df_trans_hist_ch1.query("(customer_id in @l_cust)")
    df_trans_recent_ch1 = df_trans_recent_ch1.query("(customer_id in @l_cust)")
    df_trans_yesterday_ch1 = df_trans_yesterday_ch1.query("(customer_id in @l_cust)")
    ...
    feat["n_buy_hist_idxg_gram"] = df_trans_hist.groupby(
    ...
    # postal_code * aid
    feat["postal_code_n_buy_hist"] = df_trans_hist.groupby(
        ["postal_code", "article_id"]
    )["t_dat"].agg(postal_code_n_buy_hist="count")
    feat["postal_code_n_buy_hist_short"] = df_trans_hist_short.groupby(
        ["postal_code", "article_id"]
    )["t_dat"].agg(postal_code_n_buy_hist_short="count")
    feat["postal_code_n_buy_hist_mid"] = df_trans_hist_mid.groupby(
        ["postal_code", "article_id"]
    )["t_dat"].agg(postal_code_n_buy_hist_mid="count")
    feat["postal_code_n_buy_recent"] = df_trans_recent.groupby(
        ["postal_code", "article_id"]
    )["t_dat"].agg(postal_code_n_buy_recent="count")
    feat["postal_code_n_buy_yesterday"] = df_trans_yesterday.groupby(
        ["postal_code", "article_id"]
    )["t_dat"].agg(postal_code_n_buy_yesterday="count")
    ## ch1
    feat["postal_code_n_buy_hist_ch1"] = df_trans_hist_ch1.groupby(
        ["postal_code", "article_id"]
    )["t_dat"].agg(postal_code_n_buy_hist_ch1="count")
    feat["postal_code_n_buy_recent_ch1"] = df_trans_recent_ch1.groupby(
        ["postal_code", "article_id"]
    )["t_dat"].agg(postal_code_n_buy_recent_ch1="count")
    feat["postal_code_n_buy_yesterday_ch1"] = df_trans_yesterday_ch1.groupby(
        ["postal_code", "article_id"]
    )["t_dat"].agg(postal_code_n_buy_yesterday_ch1="count")

    trans_feat_attr_list = ["index_group_no", "perceived_colour_value_id"]
    for attr in trans_feat_attr_list:
        feat[f"cust_{attr}_score_hist"] = trans_attr_feat(
            "hist", attr, df_art, df_trans_hist, sm=5
        )
        feat[f"cust_{attr}_score_recent"] = trans_attr_feat(
            "recent", attr, df_art, df_trans_recent, sm=3
        )

    emb_feat = pd.read_feather("../input/nari/article_id_umap.feather")
    emb_feat["article_id"] = emb_feat["article_id"].astype(int)
    emb_feat.index = emb_feat["article_id"]
    emb_feat.index.name = "article_id"
    emb_feat = emb_feat.drop(columns=["article_id"])
    feat[f"article_id_umap"] = emb_feat

    emb_feat = pd.read_feather("../input/nari/product_code_umap.feather")
    emb_feat.index = emb_feat["product_code"]
    emb_feat.index.name = "product_code"
    emb_feat = emb_feat.drop(columns=["product_code"])
    feat[f"product_code_umap"] = emb_feat

    emb_feat = pd.read_feather("../input/nari/customer_id_umap.feather")
    emb_feat.index = emb_feat["customer_id"]
    emb_feat.index.name = "customer_id"
    emb_feat = emb_feat.drop(columns=["customer_id"])
    feat[f"customer_id_umap"] = emb_feat

    emb_feat = pd.read_csv("../save/art_phash_svd.csv")
    emb_feat.index = emb_feat["article_id"]
    emb_feat.index.name = "article_id"
    emb_feat = emb_feat.drop(columns=["article_id"])
    feat[f"art_phash_svd"] = emb_feat

    # cluster
    emb_feat = pd.read_csv("../save/nari_art_kmeans.csv")
    emb_feat.index = emb_feat["article_id"]
    emb_feat.index.name = "article_id"
    emb_feat = emb_feat.drop(columns=["article_id"])
    emb_feat = emb_feat.add_prefix("art_cluster_")
    feat[f"art_cluster"] = emb_feat

    emb_feat = pd.read_csv("../save/nari_cust_kmeans.csv")
    emb_feat.index = emb_feat["customer_id"]
    emb_feat.index.name = "customer_id"
    emb_feat = emb_feat.drop(columns=["customer_id"])
    emb_feat = emb_feat.add_prefix("cust_cluster_")
    feat[f"cust_cluster"] = emb_feat
...
def add_feat(df, ds, de, dsr, der, dsh, deh, feat):
    ...
    for col in [
        ...
        "cust_index_group_no_score_hist",
        "cust_index_group_no_score_recent",
        "cust_perceived_colour_value_id_score_hist",
        "cust_perceived_colour_value_id_score_recent",
        "customer_id_umap",
        "cust_cluster",
    ]:
        if dev == "gpu":
    ...
    for col in [
        ...
        df = df.merge(
            feat[col],
            how="left",
            left_on=["postal_code", "article_id"],
            right_index=True,
        )
...
def recommend_pred(series_cust, day_start_val, eval_mid=True, sub_i=-1):
    ...
    for iter_art in tqdm(range(len(list_sl) - 1)):
        ...
        df_test.to_pickle(f"{save_path}/df_test_500.pkl")
        print(fdsa)
