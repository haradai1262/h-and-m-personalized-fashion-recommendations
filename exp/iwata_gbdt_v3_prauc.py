...
def recommend_train(day_start_val):
    ...
    for iter_train in tqdm(range(n_iter)):
        ...
        del list_df_nobuy
        print(list_train[0]["target"].value_counts())
        ...
        pd.to_pickle(list_model, f"{save_path}/models.pkl")
...
def recommend_pred(series_cust, day_start_val, eval_mid=True):
    ...
    for iter_train in tqdm(range(n_iter)):
        ...
        for iter_art in tqdm(range(len(list_sl) - 1)):
            list_model = pd.read_pickle(f"{save_path}/models.pkl")
        ...
        if eval_mid:
            ...
            print(f"N_iter:{iter_train+1},mapk:{mapk_val} ")
            del df_ans_tmp
...
recommend_train(day_start_val=datetime.datetime(2020, 9, 23))
