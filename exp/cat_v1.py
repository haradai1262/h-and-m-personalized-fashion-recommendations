...
CAT_PARAMS["loss_function"] = "YetiRank"
CAT_PARAMS["eval_metric"] = "MAP:top=12"
...
def add_feat(df, ds, de, dsr, der, dsh, deh, feat):
    ...
    for col in [
        ...
        df = df.merge(feat[col], how="left", left_on=["article_id"], right_index=True)
    ...
    for col in [
        ...
        df = df.merge(feat[col], how="left", left_on=["product_code"], right_index=True)
    ...
    for col in [
        ...
        df = df.merge(feat[col], how="left", left_on=["customer_id"], right_index=True)
...
def recommend_pred(series_cust, day_start_val, eval_mid=True):
    ...
    for iter_train in tqdm(range(n_iter)):
        ...
        for iter_art in tqdm(range(len(list_sl) - 1)):
            ...
            df_test.to_pickle(f"{save_path}/df_test_{iter_art}.pkl")
