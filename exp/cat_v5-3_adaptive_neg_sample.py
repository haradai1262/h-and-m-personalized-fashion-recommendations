...
def feat_store(df_trans, l_cust, ds, de, dsr, der, dsh, dsh_short, dsh_mid, deh):
    ...
    df_trans_hist_all = df_trans.query("(t_dat <= @deh)")  # all
    ...
    df_trans_hist_all = df_trans_hist_all.query("(customer_id in @l_cust)")
    ...
    feat["rate_sales_channel_hist_all"] = df_trans_hist_all.groupby(["customer_id"])[
        "sales_channel_id"
    ].agg(rate_sales_channel_hist_all="mean")
    ...
    feat["n_buy_hist_all_tran"] = df_trans_hist_all.groupby(
        ["customer_id", "article_id"]
    )["t_dat"].agg(n_buy_hist_all_tran="count")
    ...
    feat["days_after_buy_all_tran"] = df_trans_hist_all.groupby(
        ["customer_id", "article_id"]
    )["t_dat"].agg(days_after_buy_all_tran=lambda x: (ds - max(x)).days)
...
def recommend_train(day_start_val):
    ...
    for i in range(len(day_start)):
        ...
        # list_df_buy[i]["target"] = 1 * (1.0 - 0.1 * i)
    ...
    for iter_train in tqdm(range(n_iter)):
        ...
        list_train = []
        for i in range(len(day_start)):
            cust_buy_num = list_df_buy[i]["customer_id"].value_counts().to_dict()
                        ...
                        pd.DataFrame(
                            {
                                ...
                                "article_id": random.sample(
                                    top_art_all, min(640, nobuy * cust_buy_num[x])
                                ),
                            }
                        )
