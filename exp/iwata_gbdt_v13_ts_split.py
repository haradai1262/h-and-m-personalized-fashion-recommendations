...
def feat_store(df_trans, l_cust, ds, de, dsr, der, dsh, deh):
    ...
    df_buy1 = (
        df_trans_hist.groupby("product_code")["customer_id"]
        .nunique()
        .reset_index()
        .rename(columns={"customer_id": "cnt_buy1"})
    )
    df_buy2 = df_trans_hist[
        df_trans_hist.duplicated(["customer_id", "product_code"])
    ].copy()
    df_buy2 = df_buy2.drop_duplicates(["customer_id", "product_code"])
    df_buy2 = (
        df_buy2.groupby("product_code")["product_code"]
        .agg(cnt_buy2="count")
        .reset_index()
    )
    df_buy = pd.merge(df_buy1, df_buy2, how="left", on="product_code").fillna(0)
    df_buy["code_rebuy_rate"] = df_buy["cnt_buy2"] / df_buy["cnt_buy1"]
    feat["code_rebuy_rate"] = df_buy[["product_code", "code_rebuy_rate"]]
...
def add_feat(df, ds, de, dsr, der, dsh, deh, feat):
    ...
    # art_buy_hist_ch1
    df = df.merge(
        feat["art_buy_hist_ch1"], how="left", left_on=["article_id"], right_index=True
    )
    # art_buy_hist_ch2
    df = df.merge(
        feat["art_buy_hist_ch2"], how="left", left_on=["article_id"], right_index=True
    )
    # art_buy_recent
    df = df.merge(
    ...
    # code_buy_hist
    df = df.merge(
        feat["code_buy_hist"], how="left", left_on=["product_code"], right_index=True
    )
    # code_buy_recent
    df = df.merge(
        feat["code_buy_recent"], how="left", left_on=["product_code"], right_index=True
    )
    # code_buy_yesterday
    df = df.merge(
        feat["code_buy_yesterday"],
        how="left",
    ...
    df = df.merge(feat["code_rebuy_rate"], how="left", on="product_code")
...
def recommend_train(day_start_val):
    ...
    for iter_train in tqdm(range(n_iter)):
        ...
        for i in tqdm(range(len(day_start))):
            ...
            with t.timer(f"add_feat {iter_train} {i}"):
        ...
        d_tr = lgb.Dataset(X_train, label=y_train, free_raw_data=False)
        d_va = lgb.Dataset(X_val, label=y_val, free_raw_data=False)
        ...
        del X_train, y_train, X_val, y_val, d_tr, d_va
        gc.collect()
...
if mode == "cv":
    ...
    if os.path.exists(f"{cache_dir}/df_agg_val_1_{day_start_val_str}.pkl"):
        ...
        df_agg_val_1 = pd.read_pickle(
            f"{cache_dir}/df_agg_val_1_{day_start_val_str}.pkl"
        )
    else:
        df_trans = pd.read_csv(path + "transactions_train.csv", dtype=tran_dtypes)
        df_trans["t_dat"] = pd.to_datetime(df_trans["t_dat"], format="%Y-%m-%d")
        ...
        df_agg_val_1 = (
            df_trans_val_1.groupby("customer_id")["article_id"]
            .apply(list)
            .reset_index()
        )
